%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name  code  : Inversion_two_variables_t_known_resampling.m
% Version     : 20.01.2020
...
%% Input of the known age in year ad conversion to seconds

t_known          = 11;                         % Input the known age of the sample in years
t                = t_known*365.25*24.0*3600.0;  % Conversion in seconds

%% Ordering of the imported data

nC1          =51;                     % Disc number for core 1
%nC2          = 47;                     % Disc number for core 2
ind_plt      =38;                     % at which index the plateaus starts considering all cores as one single dataset
...
x_model      = 0:0.25:50;           % create a synthetic depth version [mm]
...
SP_M        = SP_t_M_ok/t;
SP_1su      = SP_t_1su_ok/t;
SP_1sd      = SP_t_1sd_ok/t;
SP_2su      = SP_t_2su_ok/t;
SP_2sd      = SP_t_2sd_ok/t;

disp(['SP x t Median     = ' num2str(SP_t_M_ok,3) ]);
disp(['SP x t 1sigma sup = ' num2str(abs(SP_t_M_ok-SP_t_1su_ok),3)]);
disp(['SP x t 1sigma inf = ' num2str(abs(SP_t_M_ok-SP_t_1sd_ok),3)]);
disp(['SP x t 2sigma sup = ' num2str(abs(SP_t_M_ok-SP_t_2su_ok),3)]);
disp(['SP x t 2sigma inf = ' num2str(abs(SP_t_M_ok-SP_t_2sd_ok),3)]);

disp(['SP Median     = ' num2str(SP_M,3) ' s-1']);
disp(['SP 1sigma sup = ' num2str(abs(SP_M-SP_1su),3) ' s-1']);
disp(['SP 1sigma inf = ' num2str(abs(SP_M-SP_1sd),3) ' s-1']);
disp(['SP 2sigma sup = ' num2str(abs(SP_M-SP_2su),3) ' s-1']);
disp(['SP 2sigma inf = ' num2str(abs(SP_M-SP_2sd),3) ' s-1']);
...
%% Conversion matrix SP_t [dimensioneless] into SP [s-1]
...
title(colorbar,'Likelihood')
hold on

SP_vec_plot      = linspace(mu_min,mu_max,TT);
SP_M_line_plot   = SP_M.*ones(size(SP_vec_plot));

mu_vec_plot      = linspace(SP_min,SP_max,TT); 
...
title('Fit Likelihood - Core 3-8 Bottom, Scan')
